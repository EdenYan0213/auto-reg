...
def verify_grok2api(
    ...
    """验证 grok2api 连通性与鉴权（新版：admin 用户名 + 密码登录）。

    app_key 参数兼容旧调用方，语义为管理密码（grok2api_app_key 配置项）。
    """
    api_url = str(api_url or _get_config("grok2api_url", "")).strip()
    password = str(app_key or _get_config("grok2api_app_key", "")).strip()
    username = str(username or _get_config("grok2api_admin_user", "admin")).strip()

    if not api_url:
        return False, "grok2api URL 未配置"
    if not password:
        return False, "grok2api 管理密码未配置"

    try:
        resp = requests.post(
            f"{api_url.rstrip('/')}/api/admin/v1/auth/login",
...
def ensure_grok2api_ready() -> Tuple[bool, str]:
    ...
    password = _get_config("grok2api_app_key", "")
    username = _get_config("grok2api_admin_user", "admin")

    ok, msg = verify_grok2api(api_url=api_url, app_key=password, username=username)
    if ok:
        return True, msg
    ...
    return verify_grok2api(api_url=api_url, app_key=password, username=username)