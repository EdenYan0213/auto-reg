...
class RegistrationEngineFlowTests(unittest.TestCase):
    ...
    def test_run_restarts_login_after_new_registration(self):
        ...
        with mock.patch.object(engine, "_check_ip_location", return_value=(True, "US")), \
            ...
            mock.patch.object(engine, "_start_passwordless_signup", return_value=False) as passwordless, \
    ...
    def test_run_passwordless_signup_skips_password(self):
        engine = self._make_engine()
        ...
        with mock.patch.object(engine, "_check_ip_location", return_value=(True, "US")), \
            ...
            mock.patch.object(engine, "_prepare_authorize_flow", return_value=("did", "sentinel")), \
            mock.patch.object(engine, "_submit_signup_form", return_value=SignupFormResult(success=True, page_type="create_account_password")), \
            mock.patch.object(engine, "_start_passwordless_signup", return_value=True) as passwordless, \
    ...
    def test_check_sentinel_falls_back_to_pow_when_browser_unavailable(
        self, mock_browser_token, mock_pow_token
    ):
        engine = self._make_engine()
        engine.session = mock.Mock()
        ...
        mock_browser_token.assert_called_once()
        mock_pow_token.assert_called_once()

    @mock.patch.object(RefreshTokenRegistrationEngine, "_check_sentinel", return_value=None)
    def test_register_password_does_not_submit_without_browser_sentinel(self, _check_sentinel):
        engine = self._make_engine()
        engine.email = "user@example.com"
        engine.session = mock.Mock()
        engine.session.get.return_value.status_code = 200
