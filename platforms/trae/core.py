...
class TraeRegister:
    ...
    def step5_get_token(self):
        """获取用户 token，返回完整原始响应（含 RefreshToken/过期时间等全部字段）"""
    ...
    def register(self, email: str, password: str = None,
        ...
        token_resp = self.step5_get_token()
        # 兼容多种返回结构：直接取 Result，或 Result.Token
        result = token_resp.get("Result", token_resp) if isinstance(token_resp, dict) else {}
        token = result.get("Token", "") if isinstance(result, dict) else ""
        self.step6_check_login()
        ...
        return {
            ...
            "ai_pay_host": (result.get("AIPayHost", "") if isinstance(result, dict) else ""),
            "raw_token_resp": token_resp,  # 完整原始响应，含 RefreshToken/过期时间
        }
