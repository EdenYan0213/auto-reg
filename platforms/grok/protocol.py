...
from .browser_bridge import XaiBrowserBridge, XaiBrowserBridgeError
from .session_cookies import apply_cookie_jar, snapshot_session_cookie_jar
...
class GrokProtocolClient:
    ...
    def _discover(self, html: str, *, force: bool) -> SignupMetadata:
        ...
        with _DISCOVERY_LOCK:
            ...
            browser_flow_enabled = str(self.config.get("browser_flow_enabled") or "").strip().lower() not in {
                "",
                "0",
                "false",
                "no",
            ...
            for script_url in reversed(script_urls):
                if action_id and (sdk_source or browser_flow_enabled):
                    break
                ...
                # 同会话浏览器流程由 8877 solver 负责 Castle/Turnstile，协议客户端只需要
                # action id、router tree 和 sitekey；不要把普通应用 chunk 误认成 Castle SDK。
                if browser_flow_enabled:
                    continue
                if not sdk_source:
            ...
            if not sdk_source and not browser_flow_enabled and _VENDOR_SDK_PATH.is_file():
                ...
                if "createRequestToken" in fallback and "configure" in fallback:
                    ...
                    sdk_source = fallback
            if not sdk_source and not browser_flow_enabled:
                raise GrokProtocolError("未能从注册页发现 Castle SDK", stage="bootstrap", retryable=True)
            sdk_path = self._cache_castle_sdk(sdk_source, castle_sdk_url) if sdk_source else ""
